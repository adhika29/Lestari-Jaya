    ...
    <div class="bg-white p-3 rounded-lg shadow-sm mb-4">
        <div class="flex items-center text-sm text-gray-600">
            ...
            <svg class="w-3 h-3 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7-7m0 0l7-7m-7 7h18"></path>
            </svg>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="mb-6">
            <div class="flex items-center mb-4">
                <a href="{{ route('biaya-konsumsi.index') }}" class="mr-4">
                    <svg class="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                </a>
                <h2 class="text-xl font-semibold">Edit Data Biaya Konsumsi</h2>
            </div>
        ...
        <form action="{{ route('biaya-konsumsi.update', $biayaKonsumsi) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $biayaKonsumsi->tanggal->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brown-500" required>
                </div>
                ...
                <div>
                    ...
                    <select id="keterangan" name="keterangan" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brown-500" required>
                        ...
                        @foreach($keteranganList as $item)
                            <option value="{{ $item }}" {{ old('keterangan', $biayaKonsumsi->keterangan) == $item ? 'selected' : '' }}>{{ $item }}</option>
                        @endforeach
                        @if(!in_array(old('keterangan', $biayaKonsumsi->keterangan), $keteranganList->toArray()))
                            <option value="{{ old('keterangan', $biayaKonsumsi->keterangan) }}" selected>{{ old('keterangan', $biayaKonsumsi->keterangan) }}</option>
                        @endif
                    </select>
                ...
                <div>
                    ...
                    <input type="number" id="volume" name="volume" value="{{ old('volume', (int)$biayaKonsumsi->volume) }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brown-500" required min="1" step="1">
                </div>
                ...
                <div>
                    ...
                    <select id="satuan" name="satuan" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brown-500" required>
                        ...
                        <option value="porsi" {{ old('satuan', $biayaKonsumsi->satuan) == 'porsi' ? 'selected' : '' }}>porsi</option>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" id="harga" name="harga" value="{{ old('harga', (int)$biayaKonsumsi->harga) }}" class="w-full pl-12 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brown-500" required min="0" step="1">
                ...
                <input type="hidden" id="total_harga" name="total_harga" value="{{ old('total_harga', $biayaKonsumsi->total_harga) }}">
        ...
        templateResult: function(data) {
            ...
            if (data.newTag) {
                return $('<span style="color:rgb(255, 255, 255); font-weight: bold;">' + 
                        data.text + ' <em style="color:rgb(255, 255, 255); font-style: italic;">(Nama Baru)</em></span>');
            }
